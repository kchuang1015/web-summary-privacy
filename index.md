...
**Non-persistent session context (`chrome.storage.session`)**: so that you can keep asking follow-ups after the Service Worker is recycled, and the side panel can restore its view when reopened, the Extension keeps **three** kinds of data in `chrome.storage.session`:
