...
- Both of this version's paths — your own key (BYOK) and Chrome built-in AI — run with no developer-operated server and no intermediary (summary and translation content goes only to the AI endpoint you configure; with Chrome built-in AI it stays fully on-device. The single exception is the "Optional public knowledge-base lookup (Wikidata)" below — **off by default**, and it sends one concept name only when you click, never any page content)
...
In every one of this version's key-storage modes, none of this is ever sent to a developer-operated server or any intermediary (only to the endpoint YOU configured, and only when you use a cloud endpoint). We still recommend:
...
- **Full-text index (`kbLex*` / `kbChunk_*`)**: term → record number and term frequency, plus paragraph boundaries — **no original text**, rebuildable at any time; both "Clear knowledge-base data" and "Clear all" delete it.
...
- **Side-panel replay log (`panel_*`)**: the already-rendered result messages in side-panel display mode, so the panel can replay after a reopen / SW restart. No URL. Removed on a real navigation (you move to a different page) or when you close the tab (reloading the same URL does not clear it, and a same-document History API change does not count, as with `conv_*`); "clear everything" **actively removes it and verifies by reading back**.
...
On both of this version's AI paths, all of this stays **on your device only** — never sent to a developer-operated server or intermediary. Exactly three things send any of it to the AI endpoint you chose (or to on-device Chrome built-in AI): **auto-tags**, which run **automatically** after each summary (the summary excerpt, up to ~4,000 characters; can be turned off in Settings); the standalone **suggested-questions** call, which also runs **automatically** and sends the same summary excerpt (up to ~4,000 characters) on selection summaries, when the question section of a full-page summary fails to parse, on Chrome built-in AI's two-stage flow, and when a cache hit finds a record that has no questions yet (can be turned off in Settings; on the cloud path a full-page summary produces the questions inside the same single summary call and sends nothing extra); and the content sent when you actively use topic pages / mind maps / knowledge Q&A. There is one further optional path that **does not go to an AI endpoint and is off by default** (next section): when you click "Find public mapping" by hand, a single concept name is sent to Wikidata.

---

## Optional public knowledge-base lookup (Wikidata)

The concept editor on a topic page lets you attach a public knowledge-base mapping (a Wikidata Q-number) to a tag concept. You can **paste** a Q-number yourself (zero network traffic), or turn on an option that is **off by default** to look up candidates:
...
- **What is sent**: once enabled, only at the moment you click "Find public mapping" on a topic page, one GET to `https://www.wikidata.org/w/api.php` carrying **that concept's name — one string** (in your interface language). The name is read from the concept table by the background script; it is never the content of any input box on the page.
- **What is not sent**: no article text, no summaries, no other tags, no URLs, no cookies; it is never automatic and never runs in the background after you enable it.
- **What is stored**: the response is at most 8 candidates (Q-number, label, one-line description); **only the one you pick is stored** (as "proposed" first; you can confirm or reject it), inside the local tag concept table, and it is cleared together with that table. Q-numbers produced by a model from memory are never accepted.
...
- Like other settings the switch syncs with Chrome (it takes effect on every device signed in to the same account), but it is **never written into a knowledge-base backup file** — importing a backup from another machine will not turn it on for you. For Wikidata's handling of the request see the [Wikimedia Foundation Privacy Policy](https://foundation.wikimedia.org/wiki/Policy:Privacy_policy).
...
- **Wikidata (optional, off by default)**: receives one concept name only when you click "Find public mapping" by hand — see the section above; see the [Wikimedia Foundation Privacy Policy](https://foundation.wikimedia.org/wiki/Policy:Privacy_policy)
