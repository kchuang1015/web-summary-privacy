...
- Both of this version's paths — your own key (BYOK) and Chrome built-in AI — run with no developer-operated server and no intermediary (content goes only to the AI endpoint you configure; with Chrome built-in AI it stays fully on-device)
...
On both of this version's AI paths, all of this stays **on your device only** — never sent to a developer-operated server or intermediary. Exactly two things send any of it to the AI endpoint you chose (or to on-device Chrome built-in AI): **auto-tags**, which run **automatically** after each summary (the summary text; can be turned off in Settings), and the content sent when you actively use topic pages / mind maps / knowledge Q&A.
