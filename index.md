...
- **最近結果（`resultHistory`）**：摘要與選取結果（含追問問答與主題標籤），總容量可在設定調整（5–100MB、預設 20MB），超出時自動汰舊最舊的紀錄。主題標籤預設由您設定的 AI 端點自動產生（摘要完成後多一次小型呼叫，可在設定關閉），也可手動增刪改；標籤同樣**只存本機**，供「知識圖譜」頁把您的閱讀內容串起來。可在「最近結果」檢視頁**逐筆刪除**或**全部清除**。
