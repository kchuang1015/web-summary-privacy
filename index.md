...
- **Topic pages (`topicPages`)**: "Generate / update" on a topic page is a feature **you trigger manually** — it sends the saved summaries under that tag to your own configured AI endpoint to synthesize one wiki page; the result is stored **locally only**, is editable, and can be deleted. "Clear recent results & translation cache" also deletes tag relations and topic pages.
...
- **標籤關聯（`kgRelations`）**：知識圖譜頁的「AI 分析關聯」是**您手動觸發**的功能——按下時會把您的標籤清單（僅標籤文字，不含網頁內容）送到您自己設定的 AI 端點，分析出的從屬／因果關聯**只存本機**，可在關聯清單逐條刪除。
- **主題頁（`topicPages`）**：主題頁的「生成／更新」是**您手動觸發**的功能——按下時會把該標籤下已保存的摘要送到您自己設定的 AI 端點合成一頁主題 wiki；生成結果**只存本機**，可編輯、可清空刪除。「清除最近結果與翻譯快取」會一併刪除標籤關聯與主題頁。
