...
- **Follow-up conversation context (`conv_*`)**: the current tab's follow-up context — **the full extracted text of that page (up to 512 KB; only longer pages are truncated), the full summary, and every previous Q&A turn**, plus a snapshot of the model settings. **As of v1.23 this snapshot always has every API-key field stripped out** (older snapshots written by earlier versions are sanitized in place on load) — **the follow-up context snapshot never contains a key**; follow-up calls re-resolve credentials from your current live settings instead. (Note: if you opt into the "keep keys only until the browser closes" mode described above, your keys are stored in a separate item, `sessionCreds`, in this same storage area — that is a storage location you chose deliberately, and is distinct from this conversation snapshot.) No URL. Removed on a real navigation (the document is replaced) or when you close the tab — **a same-document History API change does not count** (an infinite-scroll site rolling into the next article, a single-page app switching routes: the address changes but the document is the same one, so the context stays until a real navigation); "clear everything" and the viewer's "Clear all" both **actively remove it and verify by reading back**. For medical, intranet, or otherwise sensitive pages, be aware that **the full text of the page you just summarized stays here until a real navigation (the document is replaced), the tab is closed, or you clear** — the kind of "page change" where only the address bar changes and the page is not reloaded does not remove it.
- **Side-panel replay log (`panel_*`)**: the already-rendered result messages in side-panel display mode, so the panel can replay after a reopen / SW restart. No URL. Removed on a real navigation (the document is replaced) or when you close the tab (a same-document History API change does not count, as with `conv_*`); "clear everything" **actively removes it and verifies by reading back**.
...
Every key in `chrome.storage.session` is **never synced, never written to disk, and never sent to the developer**, and **all of them are gone at the latest when the browser session ends**. "A real navigation or closing the tab clears it" is true only for `conv_*`, `panel_*`, and `trrun_*` (a real navigation means the document is replaced; a same-document History API change — infinite scroll, a single-page app switching routes — does not count, and these three keys stay until a real navigation); `bgLastRunOutcome` and `topicDraft_*` stay by design until the next overwrite, a save/discard, or "clear everything"; `sessionCreds`, `bgActiveTasks`, and the two learned tables follow their own lifecycles as described above.
